...
class MOV:
	...
	def registered_component(self, _ch, _method, _properties, body):
		...
		logging.info("Register C2 Treatment justice valuator with the identifier '%s'",self.component_id)
	...
	def unregister_component(self):
		...
		if self.component_id is not None:
				...
				logging.info("Unregisterd C2 Treatment justice valuator with the identifier '%s'",self.component_id)
				self.component_id = None
