...
class EMailReplierGenerator:
    ...
    def generate_reply(self, subject: str, content: str) -> Tuple[str, str]:
        ...
        # Robust parsing: The model might return the full prompt + generation, 
        # or just the generation depending on the pipeline/model type.
        # We look for common markers to extract only the assistant's part.
        markers = ['<|assistant|>', '<|im_start|>assistant', 'assistant\n']
        reply_content = full_text
        
        for marker in markers:
            if marker in full_text:
                # Get everything after the LAST occurrence of the marker (in case of multi-turn simulated in prompt)
        ...
        for end_marker in ['<|im_end|>', '<|end|>', '</s>', '<|file_separator|>']:
            if end_marker in reply_content:
                reply_content = reply_content.split(end_marker)[0].strip()

        reply_subject = f"Re: {subject}"
        if reply_content.startswith('Subject:'):
            lines = reply_content.split('\n', 1)
            reply_subject = lines[0][len('Subject:'):].strip()
